...
// Fan enable threshold //Fans are not controlled by the BMS slave boards, but the Master board
...
/*Fan control //Fans are not controlled by the BMS slave boards, but the Master board
void setFan(bool on) {
  return; //TODO
}*/
...
void loop() {
  ...
  for (int i = 1; i < 16; i++) {
    if ((hold_array[i] < hold_low) && (hold_array[i] != 0)) {
  ...
  /*//Set fan state //Fans are not controlled by the BMS slave boards, but the Master board
